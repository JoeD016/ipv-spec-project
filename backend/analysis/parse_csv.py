...
def get_mac_addresses(mac_df, analysis_object):
    ...
    analysis_object["mac_addresses"] = list(set(mac_df["Address A"].unique()) | set(mac_df["Address B"].unique())).sort() 
    return

def get_duration_hist(mac_df, analysis_object, uuid):
    ...
    plt.savefig(f"../site/public/images/{uuid}/duration_histogram.png")

    plt.clf()
    plt.close(fig)

    analysis_object["duration_histogram"] = f"/public/images/{uuid}/duration_histogram.png"
    return 

def get_packets_sent_and_received(mac_df, analysis_object, uuid):
    ...
    for i in range(len(mac_df)):
        ...
        if sour in num_packets:
            ...
            num_packets[sour] = [mac_df.iloc[i]['Packets A â B'], mac_df.iloc[i]['Packets B â A']]
        ...
        # add to the number of packets sent by des/B
            ...
            num_packets[des] = [mac_df.iloc[i]['Packets B â A'], mac_df.iloc[i]['Packets A â B']]

    num_packets = {key: val for key, val in sorted(num_packets.items(), key = lambda ele: ele[0])}
    ...
    plt.savefig(f"../site/public/images/{uuid}/packets_sent_and_received.png")

    plt.clf()
    plt.close(fig)

    analysis_object["packets_sent_and_received"] = f"public/images/{uuid}/packets_sent_and_received.png"
    return 

def get_bandwidth_plots(network_df, analysis_object, uuid):
    # Find the time range for the packet capture
    end_time = math.ceil(network_df["Time"].iloc[-1]) + 1

    unique_addrs = list(set(network_df["Source"].unique()) | set(network_df["Destination"].unique()))
    ...
    for addr in unique_addrs:
        ...
        plt.xlabel("Time (s)")
        plt.ylabel("Bytes/s")
        plt.savefig(f"../site/public/images/{uuid}/{addr}.png")
        
        plt.clf()
        plt.close(fig)

        bandwidth_graphs[addr] = f"public/images/{uuid}/{addr}.png"
...
def parse_csv(networkActivityCSV, macAddressCSV):
    ...
    os.mkdir(f"../site/public/images/{random_id}")

    analysis_object = {uuid: random_id}
